 ...
 * 사용자 가상 주소 userpage에서 커널 가상 주소 kernelpage로의 매핑을\
 * 페이지 테이블에 추가합니다.
 * wirtable이 참이면 사용자 프로세스가 페이지를 수정할 수 있습니다
 * 그렇지 않으면 읽기 전용입니다.
 * 사용자 페이지가 이미 매핑되어 있지 않아야 합니다.
 * kernelpage는 아마도 palloc_get_page()로 생선된 사용자 풀에서 얻은 페이지여야 합니다.
 * 성공하면 true를 반환하고, 사용자 페이지가 이미 매핑되었거나 메모리 할당에 실패하면
 *  false를 반환합니다. */
static bool install_page(void *upage, void *kpage, bool writable) {
    struct thread *t = thread_current();
...
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage, uint32_t read_bytes, uint32_t zero_bytes, bool writable) {
    ...
    while (read_bytes > 0 || zero_bytes > 0) {
    	...
    	struct send_data_via *aux_info = (struct send_data_via *)malloc(sizeof(struct send_data_via));

        /* Do calculate how to fill this page.
         * We will read PAGE_READ_BYTES bytes from FILE
         * and zero the final PAGE_ZERO_BYTES bytes. */
        // printf("이번 내리실 역은 load_segment역입니다 \n");
        size_t page_read_bytes = read_bytes < PGSIZE ? read_bytes : PGSIZE;
        size_t page_zero_bytes = PGSIZE - page_read_bytes;

        aux_info->file = file;
        // printf("file_name  %X\n",file);
        aux_info->position = position;
        ...
        /* lazy_laod_segment에 정보를 전달해주기위해 aux를 세팅한다 */
        void *aux = aux_info;
        if (!vm_alloc_page_with_initializer(VM_ANON, upage, writable, lazy_load_segment, aux)) {
            // printf("load segment 안의 vm 이니셜이에요\n");
            return false;
        }
        // printf("load segment 안의 vm 이니셜 이후에요\n");
        /* Advance. */
        read_bytes -= page_read_bytes;
        zero_bytes -= page_zero_bytes;
        upage += PGSIZE;
        // aux->position += page_read_bytes;
		position += page_read_bytes;
        // printf("이거는 position 값입니다 %d\n", aux->position);
    }
    // printf("나가라\n");
    return true;
}
